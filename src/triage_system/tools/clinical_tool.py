...
class ClinicalPackagesTool:
    def __init__(self, state: PatientSessionState):
        ...
        self.state = state
        self._package_db = {
            "Cardiology": ["Electrocardiogram (ECG)", "Echocardiogram (Heart Ultrasound)", "Complete Blood Count (CBC)"],
            ...
            "Gastroenterology": ["Abdominal Ultrasound", "Liver Function Panel Blood Test", "H. Pylori Stool Test"],
    ...
    def fetch_department_examination_packages(self, department: str) -> List[str]:
        procedures = self._package_db.get(department, ["Standard General Physician Consultation"])
